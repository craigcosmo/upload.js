class Upload {
	...
	uploadFile(file) {
		...
		xhr.onload = () => {
			...
			if (
				this.completedCollection.length ===
				this.ongoingCollection.length
			) {
		...
		if (this.option.formData === false) {
			...
			let FD = new FormData()
			// let Form = this.option.form
			// let input = Form.getElementById('upload')

			let category = 'jackpot'
	...
	validate(file) {
		...
		if (
			this.option.allowedType.length &&
			!this.option.allowedType.match(type)
		) {
		...
		if (
			...
			parseInt(file.size / 1024) > this.option.maxSize
		) {
		...
		this.getImageSize(file, (file, width, height) => {
			if (this.option.minWidth > 0 && width < this.option.minWidth) {
				this.option.onDimensionError.call(this, {
					name: file.name,
					width: width,
					height: height,
				})
			...
			if (itemCount >= this.option.maxLength) {
				//needed double check items length
				this.option.onLengthError.call(this, itemCount)
				valid = false
			}
		})
